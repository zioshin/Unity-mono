...
{
	int error = 0;
	int32_t UnityPalClose(handle, &error);
	return UnityPalSuccess(error);
}
...
int64_t UnityPalGetLength(void* handle, int *error);
...
mono_w32file_copy (gunichar2 *path, gunichar2 *dest, gboolean overwrite, gint32 *error)
{
	...
	int error1 = 0;
    result = (gboolean) UnityPalCopyFile(path, dest, (int32_t) overwrite, &error);
