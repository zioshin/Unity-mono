...
ves_icall_System_IO_MonoIO_FindFirstFile (const gunichar2 *path_with_pattern, MonoStringHandleOut file_name, gint32 *file_attr, gint32 *ioerror, MonoError *error)
{
	...
	if (hnd == INVALID_HANDLE_VALUE) {
		...
		*ioerror = mono_w32error_get_last ();
		g_free ((gunichar2 *)path_with_pattern_remapped);
		return hnd;
	}
...
ves_icall_System_IO_MonoIO_MoveFile (const gunichar2 *path, const gunichar2 *dest, gint32 *error)
{
	...
	*error=ERROR_SUCCESS;
	...
	g_free ((gunichar2 *)path_remapped);
	g_free ((gunichar2 *)dest_remapped);
					...
					gint32 *error)
{
	...
	*error = ERROR_SUCCESS;
	...
	g_free ((gunichar2 *)source_file_name_remapped);
	g_free ((gunichar2 *)destination_file_name_remapped);
	g_free ((gunichar2 *)destination_backup_file_name_remapped);
				 ...
				 gint32 *error)
{
	...
	*error=ERROR_SUCCESS;
	...
	g_free ((gunichar2 *)filename_remapped);
