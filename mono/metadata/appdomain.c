							 ...
							 gpointer user_data)
{
	...
	for (tmp = domain->domain_assemblies; tmp; tmp = tmp->next) {
		...
		if (assembly_is_dynamic (ass) || refonly != ass->ref_only || !mono_assembly_names_equal_ignore_version (aname, &ass->aname, ignore_version_and_key_when_finding_assemblies_already_loaded))
			continue;
