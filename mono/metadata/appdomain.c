							 ...
							 gpointer user_data)
{
	...
	for (tmp = domain->domain_assemblies; tmp; tmp = tmp->next) {
		...
		if (ass->dynamic || refonly != ass->ref_only || !mono_assembly_names_equal_ignore_version (aname, &ass->aname, ignore_version_and_key_when_finding_assemblies_already_loaded))
			continue;
